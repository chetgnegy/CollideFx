...
std::list<std::pair<Physical *, Physical *>> recent_collisions_;
        ...
        if ( (*it_a)->has_collisions() ){ // Only if A can collide
          ...
          while (it_b != all_.end()) {
            if ((*it_b)->has_collisions()) { // Only if B can collide
              ...
              //std::cout << "TODO: Account for angular momentum, 
...
bool Physics::collide(Physical* a, Physical* b){
  between = b->pos_ - a->pos_;
  ...
  if (between.length() < b->intersection_distance() 
    ...
    b->vel_ -= b_proj * 2;
    a->vel_ -= a_proj * 2;
    std::cout << between.length() << " " << b->intersection_distance() 
  + a->intersection_distance() << std::endl;
  }
}
